...
# API news sorting :
article_titles_for_analysis = prepare_dataset.sort_news_api_data(api_response_data, sentiment_analyzer)

# Sentiments scoring :
news_scores = sentiment_analyzer.analyze_sentiments(article_titles_for_analysis)
for new_score in news_scores:
    print(f"Title : {new_score['text']}")
    print(f"  Score (MonsterAPI) : {new_score['score']:.4f}")
