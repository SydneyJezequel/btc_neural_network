...
class DisplayResultsService:
        ...
        with torch.no_grad():
            for x_batch, y_batch in test_loader:
                ...
                for i in range(len(predictions)):
                    ...
                    """
                    # Redimensionner la prédiction en un tableau 2D à une seule colonne
                    pred_to_inverse = predictions[i].reshape(-1, 1)
                    real_to_inverse = y_batch[i].reshape(-1, 1)
                    # Appliquer la dénormalisation
                    pred_inversed = scaler.inverse_transform(pred_to_inverse)
                    real_inversed = scaler.inverse_transform(real_to_inverse)
                    # Stocker les valeurs
                    predicted_prices.extend(pred_inversed)
                    real_prices.extend(real_inversed)
    ...
    def evaluate_initial_model(self, model, test_loader, scaler, feature_cols, target_col_idx, window_width=10, start_index=0, pred_length=1, device='cpu'):
        """
        Evaluates the model on test dataset and compares predictions with actual prices.
        Plots real vs. predicted values within a given window width and starting index.
        Parameters:
