...
class PrepareDatasetService:
    ...
    """ ************** Methods to prepare dataset (formatting, scalling, adding indicators, etc.) ************** """

    def standardize_numeric_data(self, df, numeric_columns):
        """ Clean and concert numerical columns. """
    ...
    def base_data_formatting(self, initial_dataset, numeric_cols_to_clean, date_format='%d/%m/%Y'):
        """ Basic formatting (dates, sort). """
        df = initial_dataset.copy()
        # date formatting and sorting :
        df['Date'] = pd.to_datetime(df['Date'], format=date_format, errors='coerce')
        df = df.sort_values(by='Date')
        # numbers formatting :
        df = self.standardize_numeric_data(df, numeric_cols_to_clean)
        return df
    ...
    def format_dataset(self, initial_dataset):
        ...
        return self.base_data_formatting(initial_dataset, numeric_columns)
    ...
    def _fit_and_transform_data(self, data):
        """ Internal utility to apply MinMax scaling to a NumPy array. """
        scaler = MinMaxScaler()
    ...
    def select_and_scale_features(self, df, feature_cols=None):
        ...
        data_scaled, scaler = self._fit_and_transform_data(data)
        return data_scaled, scaler, feature_cols



    def scale_features(self, df, columns_to_normalize_for_scaler):
        """
        Given a DataFrame, selects the relevant columns and applies MinMax scaling.
        Returns the scaled array, the fitted scaler (for inversing later), and the list of columns used.
        """
        ...
        data_scaled, scaler = self._fit_and_transform_data(data)
        df[columns_to_normalize_for_scaler] = data_scaled
        return df, scaler
    ...
    def data_formatting_for_transformer_model(self, df):
        """ Data formating """
        numeric_columns_initial = ["Dernier", "Ouv.", " Plus Haut", "Plus Bas", "Variation %"]
        # base data formatting :
        df = self.base_data_formatting(df, numeric_columns_initial)
        # rename columns :
        df.rename(columns={'Ouv.': 'Open', ' Plus Haut': 'High', 'Plus Bas': 'Low'}, inplace=True)
        # re-initialize index :
    ...
    """ ********************* prepare dataset for LSTM network and Transformer model ********************* """
    ...
    def prepare_many_dimensions_dataset_for_transformer_model(self, dataset, delete_columns, cutoff_date='2020-01-01', lags=None, add_volatility=False):
        ...
        # Normalization :
        columns_to_normalize_for_scaler = ['Dernier']
        tmp_dataset, scaler = self.scale_features(tmp_dataset, columns_to_normalize_for_scaler)
        # Adding lags if specified :
        if lags is not None:
            tmp_dataset = self.add_lag_features(tmp_dataset, lags)
        # Delete NaN values in datset for lags :
